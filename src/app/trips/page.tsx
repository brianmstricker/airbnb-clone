 ...
 if (loading)
  return (
   <div className="!max-w-[1500px] contain pt-5">
    ...
    <Border />
    Fetching...
    <CircleLoader color={"#2a3da8"} size={80} className="mx-auto mt-10" />
   </div>
  );
 return (
  <div className="!max-w-[1500px] contain pt-5">
   ...
   {reserves.length === 0 ? (
    <div className="min-h-screen">
     ...
     <p className="mt-1 font-light">Time to dust off your bags and start planning your next adventure</p>
     <Link href={"/"} className="px-6 py-2.5 mt-3 block w-fit border border-black font-semibold rounded-lg">
      Start searching
     </Link>
    ...
    <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-8 pb-20">
     {reserves.map((reserve: ReserveType) => (
      <Link href={`/place/${reserve.placeId}`} key={reserve.id} className="border border-gray-300 rounded-2xl">
       ...
       <h2 className="font-medium text-xl md:text-2xl px-3 pt-3 truncate">{reserve.place.name}</h2>
       <div className="flex flex-col gap-2 p-3 text-xs [@media(min-width:350px)]:text-sm [@media(min-width:450px)]:text-base [@media(min-width:640px)]:text-sm [@media(min-width:750px)]:text-base">
        <div className="flex items-center gap-1.5">
         <span className="font-semibold">Check In:</span> {new Date(reserve.checkInDate).toISOString().substring(0, 10)}
         <div className="w-[2px] h-[2px] bg-black rounded-full" />
         <span className="font-semibold">by:</span> {reserve.place.checkInTime}
        </div>
        <div className="flex items-center gap-1.5">
         <span className="font-semibold">Check Out:</span> {new Date(reserve.checkOutDate).toISOString().substring(0, 10)}
         <div className="w-[2px] h-[2px] bg-black rounded-full" />
